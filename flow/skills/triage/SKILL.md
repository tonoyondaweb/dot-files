...
These are canonical role names. By default, use these exact label strings in GitHub Issues: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. If custom label mappings exist, read them from `docs/agents/triage-labels.md` or `.flow/triage.yaml`. If labels are missing from the GitHub repository, create them with `gh label create`.
