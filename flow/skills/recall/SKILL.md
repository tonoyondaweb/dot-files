...
Transcripts live under `$AGENT_TRANSCRIPTS_DIR` / `$FLOW_TRANSCRIPTS_DIR`, or at `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl` (where `<slug>` is the workspace path with leading slash dropped and "/" replaced by "-"). Every line is one chat message.
