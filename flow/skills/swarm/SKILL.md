...
4. Pick the worker model from `swarm workers` in `~/.config/flow/models.yaml` (or `~/.cursor/rules/flow-models.mdc`) when present. Otherwise use `grok-4.6-fast-xhigh`. For a model race, name each arm's model up front.
